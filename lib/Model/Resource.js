...
export default class Resource {
  ...
  static async getById(id) {
   ...
   static async getByCityId(id) {
    const { rows } = await pool.query('SELECT * FROM resources WHERE city_id=$1', [
      id,
    ]);
  ...
  static async getAll() {
    const { rows } = await pool.query(`
    SELECT   resources.id, src_name, src_description, city_id, info, category_id, tags
    FROM resources
    LEFT JOIN cities
  ...
  static async updateById(
    ...
    $4, category_id=$5, tags=$6 WHERE id=$7 RETURNING *`,
      [newSrcName, newDescription, newCityId, newInfo, newCategoryId, newTags, id]
    );
    return new Resource(rows[0]);
  }
